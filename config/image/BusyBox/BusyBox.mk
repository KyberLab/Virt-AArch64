...
# (Required) Build type
IMAGE_BUILD_TYPE			:= BusyBox


# (Required) Fetch options
IMAGE_FETCH_OPTS			:= 
IMAGE_FETCH_URL				:= $(IMAGE_BUSYBOX_URL)
IMAGE_FETCH_REF				:= $(IMAGE_BUSYBOX_REF)
...
IMAGE_INSTALL_OPTS			:= 
IMAGE_INSTALL_LIST			:= output/initrd.cpio.gz:BusyBox-Initrd.cpio.gz
...
IMAGE_EXPORT_ENV			+= CROSS_COMPILE=/usr/local/gcc15-aarch64-none-linux-gnu/bin/aarch64-none-linux-gnu-
...
define image_postrun_config
	...
	$(Q)cd $(3) && sed -i 's/.*CONFIG_STATIC[ =].*/CONFIG_STATIC=y/g' build/.config
endef
